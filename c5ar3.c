...
void wigszprt(FILE *szofptr, char *bnam, unsigned fullsqlen) /* writes out the chromname and full seqsz on one line of FILE* pointer */
{
    fprintf(szofptr, "%s\t%u\n", bnam, fullsqlen); /* jim says they should be tab-separated */
    return;
}

void wigfprtheader(FILE *ofnamptr, char *bnam, unsigned fullsqlen)
{
...
void wigfprt(FILE *ofnamptr, char *bnam, unsigned stepsz, float *gcfra, unsigned totwns)
{
    unsigned i;
...
void procthefile(char *filename, unsigned winsz, FILE *wigfptr, FILE *szoptr)
{
	unsigned bnlen;
    ...
    wigszprt(szoptr, bnam, sqidx);
    wigfprt(wigfptr, bnam, stepsz, gcfra, cou); /* that cou is the total number of windows at this stage */
...
int main(int argc, char *argv[])
{
	...
	/* in this program we're going to everything in just two files. The bigwig will have */
    FILE *wigfptr=fopen("all.wig", "w");
    FILE *szoptr=fopen("all.sizes", "w");
        ...
        if(direntp->d_type == DT_REG) {
            if( (ppoi=strrchr(direntp->d_name, '.')) != NULL)
                if(strncmp((ppoi+1), argv[2], extsz)==0)
                    procthefile(direntp->d_name, winsz, wigfptr, szoptr);
        }
