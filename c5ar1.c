...
{
	if(argc!=3) {
		printf("Error. Pls supply 2 arguments: 1) name of text file 2) size of sliding window.\n");
		exit(EXIT_FAILURE);
	}
	unsigned bnlen;
	char *bnam=basn(argv[1], &bnlen);
	FILE *fin=fopen(argv[1], "r");
	int c;
    unsigned stepsz=10;
	size_t km=atoi(argv[2]);
	if(km<2) {
		printf("Error. The sliding window must be 2 or more.\n");
		exit(EXIT_FAILURE);
	}
